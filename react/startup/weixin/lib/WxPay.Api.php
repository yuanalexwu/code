...
class WxPayApi
{
	 ...
	 * @param bool $ca   是否使用ca整数认证, 默认不使用
	 * @throws WxPayException
	 */
	private static function postXmlCurl($xml, $url, $useCert = false, $second = 30)
	{		

		Log::DEBUG('[3] get prepay_id url: ' . $url);
		Log::DEBUG('[3] get prepay_id xml: ' . $xml);
		$ch = curl_init();
		//设置超时
		...
		//返回结果
		if($data){
			Log::DEBUG('[3-1] get prepay_id returns: ' . $data);
			curl_close($ch);
			return $data;
		} else { 
			$error = curl_errno($ch);
			curl_close($ch);
			Log::DEBUG('[3-2] get prepay_id errors: ' . $error);
