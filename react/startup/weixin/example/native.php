...
$row=$_Online_pay->searchNews("",array('no'=>$_GET['no']),1,10);
...
$input->SetBody('家易乐充值：' . $order_sn);
// $input->SetAttach("test");
$input->SetOut_trade_no($order_sn . '_' . time());
$input->SetTotal_fee($order_amount);
$input->SetTime_start(date("YmdHis"));
// 10分种有效期
$input->SetTime_expire(date("YmdHis", time() + 600));
// $input->SetGoods_tag("test");
$input->SetNotify_url("http://www.csckl.com/ytj/phone/wzf/example/notify.php");
...
<title>微信支付-<?php echo '家易乐账户充值：' . $order_sn; ?></title>
...
<br/>
<div  style="display: inline-block;width: 294px;line-height: 30px;height: 30px; "> <img src="../../../../img/wx.jpg" width="294" height="77"></div>
</div>
</body>
<script>
	 ...
	 function  shuaxing(){
	 $.getJSON("/pay_2.php", { Id:Math.random(),no:'<?=$row[0]['no']?>'}, function(date){
	  newriqi=date.fanhui;
	  
	
	  if(newriqi=='ok'){
		  alert('支付成功!');
		  window.parent.location.href = '<?php echo "/member_pay.php";?>';
	  }
	 });
 }
